...
export default function SanitizedTextContent({ text }: SanitizedTextContentProps) {
  ...
  function transform(node: HTMLElement, children: Node[]): React.ReactNode {
    if (node.tagName === 'a' && !node.getAttribute('href')?.includes('script')) {
      if (internalTypes.includes(node.getAttribute('data-type') as string)) {
        return (
          <Link
            passHref
            href={`/terminology-api/api/v1/resolve?uri=${node.getAttribute('href')}`}
          >
        ...
        return (
          <Link passHref href={node.getAttribute('href') ?? ''}>
            <SuomiExternalLink href='' labelNewWindow={`${t('link-opens-new-window-external')} ${children}`} >
              {children}
            </SuomiExternalLink>
          </Link>
        );
