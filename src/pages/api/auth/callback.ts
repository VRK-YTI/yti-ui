...
export default withSession(async (req, res) => {
  ...
  const shibsession_key = Object.keys(req.cookies).find(x => x.startsWith('_shibsession'));
  ...
  try {
    ...
    const apiBase = process.env.TERMINOLOGY_API_URL ?? 'http://yti-terminology-api:9103/terminology-api';
    const apiPath = new URL(apiBase).pathname;
    let fetchUrl = proxyUrl + apiPath + '/api/v1/frontend/authenticated-user';
