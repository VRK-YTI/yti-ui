...
import { anyMatching, normalizeAsArray } from '../utils/array';
...
export class ReferencesPipe implements PipeTransform {
  ...
  transform(obj: Node<any>, showEmpty = true, rejectTypes?: string[]): Reference<any>[] {
    ...
    const references = Object.values(obj.references).filter(reference => (showEmpty || !reference.empty) && !anyMatching(rejects, reject => reference.meta.id === reject));
