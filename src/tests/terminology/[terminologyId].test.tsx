...
describe('terminologyId page', () => {
  ...
  it('should create redux state with requests', async () => {
    ...
    expect((results.props as any).initialState).toBeDefined();
    const initialState = (results.props as any).initialState;

    // check that each of the mock responses are found
    const data = Object.keys(initialState.vocabularyAPI.queries)
      .map(x => initialState.vocabularyAPI.queries[x].data);
