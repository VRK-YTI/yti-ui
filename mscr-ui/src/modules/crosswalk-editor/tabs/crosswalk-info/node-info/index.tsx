...
export default function NodeInfo(props: {
  ...
  sourceData: RenderTree[];
  targetData: RenderTree[];
  ...
  const [targetDropdownValue, setTargetDropdownValue] =
    useState(targetSelectionInit);
  const [selectedSource] = props.sourceData.filter(
    (item) => item.id === sourceDropdownValue
  );
  const [selectedTarget] = props.targetData.filter(
    (item) => item.id === targetDropdownValue
  );
      ...
      schemaAttributes.forEach((scAtt) => {
        if (Object.prototype.hasOwnProperty.call(item, scAtt)) {
          const attrib: constantAttribute = {
            name: item.name,
            title: item['title'] === '' ? 'complex' : item['title'],
          };
                    ...
                    <>
                      {selectedSource?.title && (
                        <div className="col-12">
                          <div>title:</div>
                          <div className="attribute-font">
                            {processHtmlLinks(selectedSource?.title)}
                          </div>
                        </div>
                      )}
                      {selectedSource?.type && (
                        <div className="col-12">
                          <div>type:</div>
                          <div className="attribute-font">
                            {processHtmlLinks(selectedSource?.type)}
                          </div>
                        </div>
                      )}
                      {selectedSource?.description && (
                        <div className="col-12">
                          <div>description:</div>
                          <div className="attribute-font">
                            {processHtmlLinks(selectedSource?.description)}
                          </div>
                        </div>
                      )}
                    </>
                    ...
                    <>
                      {selectedTarget?.title && (
                        <div className="col-12">
                          <div>title:</div>
                          <div className="attribute-font">
                            {processHtmlLinks(selectedTarget?.title)}
                          </div>
                        </div>
                      )}
                      {selectedTarget?.type && (
                        <div className="col-12">
                          <div>type:</div>
                          <div className="attribute-font">
                            {processHtmlLinks(selectedTarget?.type)}
                          </div>
                        </div>
                      )}
                      {selectedTarget?.description && (
                        <div className="col-12">
                          <div>description:</div>
                          <div className="attribute-font">
                            {processHtmlLinks(selectedTarget?.description)}
                          </div>
                        </div>
                      )}
                    </>
