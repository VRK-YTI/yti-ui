...
export default function MetadataFilesTable({
  filesRowInput, pid, crosswalkData,
  canEdit
}: {
  ...
  function generateDownloadUrl(fileType: DownloadTypes, pid: string, fileId?: string) {
    switch (fileType) {
      ...
      case DownloadTypes.CROSSWALK_FILES: {
        return BASEURL + '/crosswalk/' + pid + '/files/' + fileId + '?download=true';
      }
      default: {
        return '';
      }
  ...
  if (filesRowInput && pid) {
    tableRows = filesRowInput.map(
      (item: { filename: any; fileID: Number; contentType: any; size: Number }) => {
        const downloadType = crosswalkData ? DownloadTypes.CROSSWALK_FILES : DownloadTypes.SCHEMA_FILES;
        const test = {
