    ...
    <React.Fragment>
      ...
      >
        ...
        <StyledTableCell className="fw-bold" style={{ width: '10%' }}>
          <IconButton
            onClick={(e) => props.cbf.performAccordionAction(row, 'remove')}
            aria-label="unlink"
            color="primary"
            title="Unlink nodes"
            size="large"
      ...
      <StyledTableRow>
        <TableCell style={{ paddingBottom: 0, paddingTop: 0 }} colSpan={6}>
          <Collapse in={open} timeout="auto" unmountOnExit>
            <Box sx={{ margin: 1 }}>
              <div className="fw-bold mt-3 mb-2" style={{ fontSize: '0.9em' }}>
                Mapping type: <span className="fw-normal">exact match</span>
              </div>
              <Textarea
                onChange={(text) => setChangedNotes(text.toString())}
                onBlur={() =>
                  props.cbf.performAccordionAction(row, addNotes, changedNotes)
                }
                labelText="Notes:"
